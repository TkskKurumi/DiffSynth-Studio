...
class CosineWarmupRestartLR(torch.optim.lr_scheduler.LambdaLR):
    ...
    def __init__(self, optimizer, warmup_steps: int, restart_steps: int, edecay_steps: int, last_epoch: int = -1):
...
def get_scheduler(
    ...
    Args:
        ...
        edecay_steps: Exponential decay time constant (used by cosine-warmup-restart).
